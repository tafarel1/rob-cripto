...
export class TradingEngine {
  ...
  public async getStats(): Promise<any> {
    ...
    return {
      ...
      riskMetrics: {
        ...riskStats,
        totalTrades: riskStats.dailyTrades,
  ...
  private async analyzeStrategy(config: StrategyConfig): Promise<void> {
    const symbol = config.symbols?.[0] || config.symbol;
    if (!symbol) {
        console.error(`Strategy ${config.name} has no symbol defined.`);
        return;
    }
    const timeframe = config.timeframes?.[0] || config.timeframe || '1h';
    ...
    for (const signal of signals) {
      // 3. Alternative Data Analysis (Sentiment/OnChain)
      // Passar dados alternativos para refinar sinal
      // Note: Assuming refineSignalWithAlternativeData is implemented or we skip it if missing
      if (altMetrics) {
    ...
    this.dbService.saveMarketData(symbol, marketData).catch((err: any) => {
   ...
   */
  private refineSignalWithAlternativeData(signal: TradingSignal, metrics: AlternativeMetrics | null): void {
    if (!metrics) return;

    let scoreModifier = 0;
    const sentimentScore = Array.isArray(metrics.sentiment) && metrics.sentiment.length > 0
        ? metrics.sentiment.reduce((acc: number, s: any) => acc + s.score, 0) / metrics.sentiment.length
        : 0;
    ...
    if (metrics.derivatives && metrics.derivatives.fundingRate > 0.0001 && signal.type === 'SELL') scoreModifier += 0.1;
    // High Negative Funding (<-0.01%) -> Crowded Shorts -> Potential Reversal Up -> Boost BUY
    if (metrics.derivatives && metrics.derivatives.fundingRate < -0.0001 && signal.type === 'BUY') scoreModifier += 0.1;
  ...
  private async processSignal(
    ...
    const symbol = config.symbols?.[0] || config.symbol;
    if (!symbol) {
        console.error(`Cannot process signal: No symbol defined for strategy ${config.name}`);
        return;
    }
    ...
    const cachedData = this.marketDataCache.get(`${symbol}_${config.timeframe}`);
    ...
    try {
      ...
      // 3. Save Signal to DB
      this.dbService.saveSignal(symbol, signal).catch(console.error);

      // Registrar posição
      this.riskManager.registerPosition(position);
      this.activePositions.set(position.id, position);

      console.log(`Posição criada: ${position.id} - ${signal.type} ${symbol} @ ${entryPrice}`);
  ...
  private async updatePosition(position: TradePosition): Promise<void> {
    ...
    const _unrealizedPnl = position.type === 'LONG'
      ...
      if (position.type === 'LONG') {
        if (currentPrice > position.entryPrice * (1 + 0.01)) { // 1% profit
           // Move SL to Breakeven
           position.stopLoss = Math.max(position.stopLoss, position.entryPrice);
           
           // If we have an order ID for SL, we should update it on exchange
           if (position.stopLossOrderId) {
               const slSide = position.type === 'LONG' ? 'sell' : 'buy';
               const updatedOrder = await this.exchangeService.modifyOrder(
                   'binance', 
                   position.stopLossOrderId,
                   position.symbol, 
                   'stop_market', // Assuming SL is a stop market order
                   slSide,
                   position.quantity,
                   position.stopLoss
               );
  ...
  private async closePosition(position: TradePosition, reason: string): Promise<void> {
    ...
    try {
      ...
      const realizedPnl = position.type === 'LONG'
        ? (currentPrice - position.entryPrice) * position.quantity - (position.fees || 0)
        : (position.entryPrice - currentPrice) * position.quantity - (position.fees || 0);
